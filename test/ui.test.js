...
test('all page sections and navigation controls are structurally accessible',()=>{
 const app=mount();try{const sections=[...app.w.document.querySelectorAll('main > section.page')];assert.equal(sections.length,9);assert.equal(sections.filter(s=>!s.hidden).length,1);assert.equal(new Set([...app.w.document.querySelectorAll('[id]')].map(n=>n.id)).size,app.w.document.querySelectorAll('[id]').length);app.$('more-toggle').click();assert.equal(app.$('more-menu').hidden,false);assert.equal(app.$('more-toggle').getAttribute('aria-expanded'),'true');app.w.document.dispatchEvent(new app.w.KeyboardEvent('keydown',{key:'Escape'}));assert.equal(app.$('more-menu').hidden,true);}finally{app.close();}
});
