...
const CATEGORIES=['gambling','alcohol','smoking','drugs','behaviours','general'];
const empty = () => ({version:2,entries:[],plan:'',profile:{categories:[],active:'gambling'}});
...
export function validateSnapshot(d){
 if(!d||![1,2].includes(d.version)||!Array.isArray(d.entries)||d.entries.length>5000||typeof d.plan!=='string'||d.plan.length>2000)throw new Error('invalid');
 const seen=new Set();
 const entries=d.entries.map(e=>{
  const category=d.version===1?'gambling':e?.category;
  if(!CATEGORIES.includes(category)||!e||!/^\d{4}-\d{2}-\d{2}$/.test(e.date)||!Number.isFinite(Date.parse(e.date))||new Date(e.date).toISOString().slice(0,10)!==e.date||e.date>new Date(Date.now()+86400000).toISOString().slice(0,10)||seen.has(category+':'+e.date))throw new Error('date/category');
  seen.add(category+':'+e.date);const v={date:e.date,category};
  for(const k of ['urge','loneliness','social','sleep','financialStress']){if(!Number.isInteger(e[k])||e[k]<0||e[k]>10)throw new Error('rating');v[k]=e[k];}
  const behavior=category==='gambling'?'played':'engaged',tags=e.tags===undefined?[]:e.tags;
  if(typeof e[behavior]!=='boolean'||typeof e.note!=='string'||e.note.length>4000||!Array.isArray(tags)||tags.length>20||tags.some(t=>typeof t!=='string'||t.length>80))throw new Error('entry');
  return {...v,[behavior]:e[behavior],note:e.note,tags:[...tags]};
 });
 let profile;
 if(d.version===1)profile={categories:(entries.length||d.plan)?['gambling']:[],active:'gambling'};
 else {const p=d.profile;if(!p||!Array.isArray(p.categories)||p.categories.length>CATEGORIES.length||p.categories.some(c=>!CATEGORIES.includes(c))||new Set(p.categories).size!==p.categories.length||!CATEGORIES.includes(p.active)||(p.categories.length>0&&!p.categories.includes(p.active)))throw new Error('profile');profile={categories:[...p.categories],active:p.active};}
 return {version:2,entries:entries.sort((a,b)=>a.date.localeCompare(b.date)||a.category.localeCompare(b.category)),plan:d.plan,profile};
}
...
async function snapshot(db,id){const r=await db.prepare('SELECT revision,data FROM snapshots WHERE user_id=?').bind(id).first();return r?{revision:r.revision,data:validateSnapshot(JSON.parse(r.data))}:{revision:0,data:empty()};}
export async function handleApi(request,env,testHooks={}){
 ...
 const updated=await db.prepare("UPDATE snapshots SET data=?,revision=revision+1 WHERE user_id=? AND revision=? AND (?=2 OR json_extract(data,'$.version')=1) RETURNING revision").bind(JSON.stringify(data),session.user_id,b.baseRevision,b.data.version).first();
 if(!updated){const stored=await db.prepare('SELECT data FROM snapshots WHERE user_id=?').bind(session.user_id).first();if(b.data.version===1&&stored&&JSON.parse(stored.data).version===2)return response({error:'client_upgrade_required'},426);return response({error:'conflict',...await snapshot(db,session.user_id)},409);}return response({revision:updated.revision,data});}
