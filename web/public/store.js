'use strict';
(function(root){
const KEY='addiction-breaker:v1';
...
function load(storage){const raw=storage.getItem(KEY);return raw ? validate(JSON.parse(raw)) : empty();}
function save(storage,data){const value=validate(data);if(value.remember)storage.setItem(KEY,JSON.stringify(value));else storage.removeItem(KEY);return value;}
const api={KEY,today,validateEntry,validate,empty,load,save};
